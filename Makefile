...
$(BUILD_DIR)/example.o: examples/example.c
	$(CC) $(CFLAGS) -c $< -o $@

debug: $(BUILD_DIR) $(LIB_DIR) $(BIN_DIR)
	$(CC) $(CFLAGS_DEBUG) -c $(LIB_SRCS) -o $(BUILD_DIR)/memalloc.o
	ar rcs $(LIB_TARGET) $(BUILD_DIR)/memalloc.o
	$(CC) $(CFLAGS_DEBUG) -c $(EXAMPLE_SRCS) -o $(BUILD_DIR)/example.o
	$(CC) $(CFLAGS_DEBUG) -o $(EXAMPLE_TARGET) $(BUILD_DIR)/example.o $(LIB_TARGET)
	gdb ./$(EXAMPLE_TARGET)
